...
const oddValues = (arr) => {
  // Solution code here...
  let odd = arr.filter( (value) => (value%2));
  return odd;
};
...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  let vowels = arr.filter((str)=>   str.match(/[aeiou]/g)
  );
  return vowels;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let notIncluded = arr.filter( (value)=>{
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  ...
  let stat = arr.filter(arr=>  arr.baseStat > minBaseStat)
     return stat;
};
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  let names =[];
  let obj= arr.filter(obj => obj.baseStat > minBaseStat)
     for(let i=0; i<obj.length; i++){
    names.push(obj[i].stat.name) 
    }
    return names;
};
