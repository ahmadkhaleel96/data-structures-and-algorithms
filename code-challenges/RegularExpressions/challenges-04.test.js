...
const isNum = (input) => {
  // Solution code here...
  let val = /[0-9]/g;
  if (val.test(input)==true){
      return true;
  } else{
      return false;
  }
  console.log(val.test(input))
};
...
const isCapitalized = (str) => {
  // Solution code here...
//   let match = [];
//   let captial = /\b[A-Z].*?\b/g;
let capital = /\w+[A-Z]/g
  let match = str.match(capital);
  match.push(str.match(capital));
//   console.log(str.match(capital));
  return match;
};
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  let cities = /[A-J].\b/g;
  arr.forEach((value) => value.find(cities));
  return arr
};
...
const matchMonth = (input) => {
  // Solution code here...
  let month = /October|Oct|october|oct/g;
   if(!input.match(month)){
       return false;
   } else{
       return true;
   }
